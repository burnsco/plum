...
function parsePlumStructuredErrorBody(body: string): {
  ...
  const machineCode = errRaw !== "" && /^[a-z][a-z0-9_]*$/.test(errRaw) ? errRaw : undefined;
  const message =
...
function buildScanQuery(options?: {
...
export function createPlumApiClient(options: CreatePlumApiClientOptions) {
      ...
      Effect.flatMap((body) => {
        ...
        return Effect.fail(
          new ApiHttpError({
            ...
            body,
            message,
            ...(structured?.machineCode !== undefined ? { code: structured.machineCode } : {}),
          }),
        );
      }),
    );
  ...
  const effects = {
          ...
          response.status === 404
            ...
            : !response.ok
              ? failHttpEffect(
                  ...
                  "GET",
                  url,
                  ({ status, body }) => body || `Movie details: ${status}`,
                )
              : null,
      }),
          ...
          response.status === 404
            ...
            : !response.ok
              ? failHttpEffect(
                  ...
                  "GET",
                  url,
                  ({ status, body }) => body || `Show details: ${status}`,
                )
              : null,
      }),
          ...
          response.status === 404
            ...
            : !response.ok
              ? failHttpEffect(
                  ...
                  "GET",
                  url,
                  ({ status, body }) => body || `Discover title: ${status}`,
                )
              : null,
      }),
    ...
    refreshLibraryPlaybackTracks: (libraryId: number) =>
      jsonRequestEffect({
        ...
        errorMessage: ({ status, body }) => body || `Refresh library playback tracks: ${status}`,
      }),
  ...
  return {
    ...
    getDiscover: (options?: { readonly originCountry?: string }) =>
      run(effects.getDiscover(options)),
