...
class SessionRepository @Inject constructor(
    ...
    suspend fun redeemQuickConnect(code: String): Result<DeviceLoginResult> = runCatching {
    ...
    suspend fun createDeviceQuickConnectCode(): Result<String> = runCatching {
    ...
    suspend fun completeDeviceQuickConnect(code: String): Result<DeviceLoginResult?> = runCatching {
        val api = getPlumApi()
        val res = api.redeemQuickConnect(QuickConnectRedeemRequest(code = code.trim()))
        if (res.code() == 202) {
