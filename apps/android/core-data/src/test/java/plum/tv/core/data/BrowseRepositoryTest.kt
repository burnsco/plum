...
class BrowseRepositoryTest {
    ...
    fun peekContiguousLibraryMediaPages_returnsEveryCachedPageInOrder() = runBlocking {
        ...
        coEvery { api.libraryMedia(1, offset = 0, limit = 60) } returns
            Response.success(page(1, 0, 60, 60))
        coEvery { api.libraryMedia(1, offset = 60, limit = 60) } returns
            Response.success(page(2, 60, 60, null))
    ...
    private fun page(id: Int, offset: Int, limit: Int, nextOffset: Int?): LibraryMediaPageJson = LibraryMediaPageJson(
        items =
            listOf(
