...
export function PageHeader({
  ...
  return (
    <header
      className={cn(
        "flex flex-col gap-4 border-b border-(--plum-border)/80 pb-4 md:flex-row md:items-end md:justify-between",
        className,
      )}
    >
      <div className="min-w-0 space-y-2">
        <h1 className="text-[1.875rem] font-semibold tracking-[-0.03em] text-(--plum-text)">
          {title}
        </h1>
        {description ? (
          <p className="max-w-3xl text-sm leading-6 text-(--plum-muted)">{description}</p>
        ) : null}
      </div>

      {(meta || actions) && (
        <div className="flex shrink-0 flex-col items-start gap-3 md:items-end">
          {meta ? <div className="text-sm text-(--plum-muted)">{meta}</div> : null}
...
export function Surface({
  ...
  return (
    <Comp
      className={cn(
        "rounded-(--radius-xl) border border-(--plum-border) bg-(--plum-panel)/92 p-5 shadow-[0_18px_40px_rgba(3,8,20,0.14)]",
        className,
      )}
    >
      {children}
...
export function EmptyState({
  ...
  return (
    <div
      className={cn(
        "rounded-(--radius-xl) border border-dashed border-(--plum-border) bg-(--plum-panel)/55 p-8",
        className,
      )}
    >
  ...
  return (
    <span
      className={cn(
        ...
        active
          ? "border-(--plum-accent)/35 bg-(--plum-accent-soft) text-(--plum-accent)"
          : "border-(--plum-border) bg-(--plum-panel-alt)/80 text-(--plum-muted)",
        className,
      )}
    >
      {children}
