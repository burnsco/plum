...
export function RatingBadge({
  ...
  if (source === "imdb") {
    return (
      <span
        className={cn(
          "inline-flex items-center gap-1.5 text-[#f5c518]",
          isMedium ? "text-sm" : "text-[0.72rem]",
          className,
        )}
      >
        <span
          className={cn(
            "inline-flex items-center justify-center rounded-full bg-[#f5c518] font-black text-[#101010] shadow-[inset_0_-1px_0_rgba(0,0,0,0.18)]",
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 text-[#7fffd0]",
        isMedium ? "text-sm" : "text-[0.72rem]",
        className,
      )}
    >
      <span
        className={cn(
          "inline-flex items-center justify-center rounded-full bg-[linear-gradient(135deg,#90cea1_0%,#3cbec9_52%,#01b4e4_100%)] font-black lowercase text-[#032b25]",
          isMedium ? "px-2.5 py-1 text-[0.8rem]" : "px-2 py-0.5 text-[0.64rem]",
        )}
      >
        tmdb
