...
type libraryScanActivity struct {
	...
	Failure *libraryScanFailure        `json:"failure,omitempty"`
}
...
type libraryScanFailure struct {
	Phase        string `json:"phase"`
	Target       string `json:"target"`
	RelativePath string `json:"relativePath"`
...
type LibraryScanManager struct {
	...
	identifyAfterScan map[int]bool
...
func NewLibraryScanManager(sqlDB *sql.DB, meta metadata.Identifier, hub *ws.Hub) *LibraryScanManager {
	return &LibraryScanManager{
		...
		identifyAfterScan: make(map[int]bool),
...
func (m *LibraryScanManager) Recover() error {
	...
	for _, persisted := range statuses {
		...
		delete(m.identifyAfterScan, status.LibraryID)

		switch {
		case status.Phase == libraryScanPhaseQueued || status.Phase == libraryScanPhaseScanning:
			status = normalizeRecoveredScanStatus(status, recoveredAt)
			...
			delete(m.lastActivityAt, status.LibraryID)
			m.debounceReady[status.LibraryID] = true
			m.identifyAfterScan[status.LibraryID] = persisted.Type != db.LibraryTypeMusic && status.IdentifyRequested
			resumedReasons = append(resumedReasons, fmt.Sprintf("recover library=%d reason=queued-scan", status.LibraryID))
		case status.Enriching:
			identifyWasActive := status.IdentifyPhase == libraryIdentifyPhaseQueued || status.IdentifyPhase == libraryIdentifyPhaseIdentifying
			status = normalizeRecoveredScanStatus(status, recoveredAt)
			if identifyWasActive {
				status.IdentifyPhase = libraryIdentifyPhaseIdle
				status.Identified = 0
				status.IdentifyFailed = 0
			}
			...
			delete(m.lastActivityAt, status.LibraryID)
			m.debounceReady[status.LibraryID] = true
			m.identifyAfterScan[status.LibraryID] = persisted.Type != db.LibraryTypeMusic && identifyWasActive
			resumedReasons = append(resumedReasons, fmt.Sprintf("recover library=%d reason=enrichment-recovery", status.LibraryID))
		default:
			m.finalizeActivityLocked(status.LibraryID, status)
		}
		if (status.IdentifyPhase == libraryIdentifyPhaseQueued || status.IdentifyPhase == libraryIdentifyPhaseIdentifying) &&
			status.Phase == libraryScanPhaseCompleted {
			identifiesToResume = append(identifiesToResume, status.LibraryID)
			m.setActivityStageLocked(status.LibraryID, "identify", true, true)
			delete(m.lastActivityAt, status.LibraryID)
			resumedReasons = append(resumedReasons, fmt.Sprintf("recover library=%d reason=identify-recovery", status.LibraryID))
		}
	...
	for _, reason := range resumedReasons {
		log.Print(reason)
	}
...
func (m *LibraryScanManager) start(libraryID int, path, libraryType string, identify bool, subpaths []string) libraryScanStatus {
	...
	if ok && status.Phase == libraryScanPhaseScanning {
		...
		if identify && libraryType != db.LibraryTypeMusic {
			m.identifyAfterScan[libraryID] = true
		}
	...
	m.identifyAfterScan[libraryID] = identify && libraryType != db.LibraryTypeMusic
	m.setActivityStageLocked(libraryID, "queued", true, true)
	m.debounceReady[libraryID] = false
...
func normalizeRecoveredScanStatus(status libraryScanStatus, recoveredAt string) libraryScanStatus {
	wasQueued := status.Phase == libraryScanPhaseQueued
	status.Phase = libraryScanPhaseQueued
	status.Enriching = false
	if !wasQueued {
		status.QueuedAt = recoveredAt
	}
	status.StartedAt = recoveredAt
	status.FinishedAt = ""
	status.Error = ""
	return status
}
...
func coalesceError(values ...string) string {
...
func (m *LibraryScanManager) captureFailureLocked(libraryID int, errText string) {
	activity := m.activities[libraryID]
	...
	if activity.Current != nil {
		failure.Phase = activity.Current.Phase
		failure.Target = activity.Current.Target
		failure.RelativePath = activity.Current.RelativePath
		failure.At = activity.Current.At
	}
...
func (m *LibraryScanManager) finalizeActivityLocked(libraryID int, status libraryScanStatus) {
	...
	if status.Phase == libraryScanPhaseFailed || (!identifyActive && status.IdentifyPhase == libraryIdentifyPhaseFailed) {
		m.captureFailureLocked(libraryID, coalesceError(status.LastError, status.Error))
		return
	}
...
func (m *LibraryScanManager) run(libraryID int, status libraryScanStatus, libraryType, path string) {
	...
	if current := m.status(libraryID); current.Phase == libraryScanPhaseCompleted && m.consumeIdentifyAfterScan(libraryID) {
		m.startIdentify(libraryID)
	}
...
func (m *LibraryScanManager) startIdentify(libraryID int) {
	...
	m.mu.Unlock()
	logging.Event("scan", "identify_queued", logging.Fields{"library_id": libraryID})
	...
	go func() {
		...
		m.setActivityStageLocked(libraryID, "identify", true, false)
		m.mu.Unlock()
		logging.Event("scan", "identify_started", logging.Fields{"library_id": libraryID, "library_type": libraryType})
		...
		logging.Event("scan", "identify_"+outcome, logging.Fields{
			"library_id":   libraryID,
			"library_type": libraryType,
			"identified":   result.Identified,
			"failed":       result.Failed,
			"elapsed":      time.Since(startedAt).Round(time.Millisecond).String(),
			"error":        coalesceError(errText(err), status.LastError),
		})
		m.flushStatus(libraryID, true)
...
func (m *LibraryScanManager) finish(libraryID int, phase string, result db.ScanResult, errText string) {
	...
	if phase == libraryScanPhaseFailed {
		...
		m.captureFailureLocked(libraryID, errText)
	} else {
		m.clearRetryStateLocked(libraryID, &status)
	}
	m.jobs[libraryID] = status
	...
	if phase == libraryScanPhaseCompleted && hasRerun {
		...
		m.identifyAfterScan[libraryID] = rerun.identify && m.types[libraryID] != db.LibraryTypeMusic
		m.setActivityStageLocked(libraryID, "queued", true, true)
		m.debounceReady[libraryID] = false
		m.scheduleDebounceLocked(libraryID)
	...
	m.flushAllStatuses(true)
	if phase == libraryScanPhaseFailed {
		logging.Event("scan", "scan_failed", logging.Fields{"library_id": libraryID, "error": errText})
		_ = m.scheduleRetry(libraryID, false, errText)
	} else if phase == libraryScanPhaseCompleted {
		logging.Event("scan", "scan_completed", logging.Fields{
			"library_id": libraryID,
...
func (m *LibraryScanManager) startEnrichment(libraryID int, libraryType, path string, subpaths []string, tasks []db.EnrichmentTask, identifyRequested bool) {
	...
	logging.Event("scan", "enrichment_queued", logging.Fields{"library_id": libraryID, "library_type": libraryType})
	m.flushStatus(libraryID, true)
...
func (m *LibraryScanManager) consumeIdentifyAfterScan(libraryID int) bool {
	m.mu.Lock()
	defer m.mu.Unlock()
	if !m.identifyAfterScan[libraryID] {
		return false
	}
	delete(m.identifyAfterScan, libraryID)
	return true
}
...
func (m *LibraryScanManager) failEnrichment(libraryID int, errText string) {
	...
	m.finalizeActivityLocked(libraryID, status)
	m.mu.Unlock()
	logging.Event("scan", "enrichment_failed", logging.Fields{"library_id": libraryID, "error": errText})
	m.flushStatus(libraryID, true)
	_ = m.scheduleRetry(libraryID, false, errText)
}
